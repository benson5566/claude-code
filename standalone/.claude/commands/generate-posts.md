...
為「AI 學習中心」網站的內容批次生成社群媒體貼文草稿,寫入 `data/posts.data.js`。
...
1. 讀取內容資料(每項都有唯一 `id`):
   - `data/news.data.js`(新聞,`n-` 前綴)
   - `data/tutorials.data.js`(教學,`t-` 前綴)
   - `data/resources.data.js`(資源,`r-` 前綴)
2. 讀取 `data/posts.data.js`,找出**還沒有草稿**的項目。若使用者有指定 id 或「全部重新生成」,以使用者指示為準:$ARGUMENTS
...
4. 合併寫回 `data/posts.data.js`,維持既有格式:
...
5. 用 `node --check data/posts.data.js` 驗證語法。
